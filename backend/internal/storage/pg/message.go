...
func (s *Storage) addAttachments(q Querier, board domain.BoardShortName, messageID domain.MsgId, attachments domain.Attachments) error {
	for _, attachment := range attachments {
		...
		var fileId int64
		err := q.QueryRow(`
            INSERT INTO files (file_path, original_filename, file_size_bytes, mime_type, image_width, image_height, thumbnail_path)
            VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
			attachment.File.FilePath, attachment.File.OriginalFilename, attachment.File.FileSizeBytes,
			attachment.File.MimeType, attachment.File.ImageWidth, attachment.File.ImageHeight, attachment.File.ThumbnailPath,
		).Scan(&fileId)
		if err != nil {
