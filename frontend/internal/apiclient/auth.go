...
func (c *APIClient) Register(r *http.Request, email, password string) (*http.Response, error) {
	...
	return c.do(r, "POST", "/v1/auth/register", bytes.NewBuffer(jsonBody))
}

// ConfirmEmail sends an email confirmation code to the backend.
func (c *APIClient) ConfirmEmail(r *http.Request, email, code, refSource string) error {
	...
	resp, err := c.do(r, "POST", "/v1/auth/check_confirmation_code", bytes.NewBuffer(jsonBody))
	if err != nil {
		return err
	}
...
func (c *APIClient) Login(r *http.Request, email, password string) (*http.Response, error) {
	...
	return c.do(r, "POST", "/v1/auth/login", bytes.NewBuffer(jsonBody))
}
...
func (c *APIClient) RegisterWithInvite(r *http.Request, inviteCode, password, refSource string) (string, error) {
	...
	resp, err := c.do(r, "POST", "/v1/auth/register_with_invite", bytes.NewBuffer(jsonBody))
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
